...
export default function CartPage() {
    ...
    const calculteTotal = (value: ProductInCart[]) => {
        return value.reduce((sum, item) => sum += (item.price_in_cents * item.quantity), 0)
    }

    const cartTotal = formatPrice(calculteTotal(value))
    const deliveryFee = 4000
    const cartTotalWithDelivery = formatPrice(calculteTotal(value) + deliveryFee)
